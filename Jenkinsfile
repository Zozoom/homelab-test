...
"""
    ...
    stages {
        ...
        stage('Helm deploy') {
            steps {
                container('helm') {
                    sh 'helm upgrade --install homelab-test ./helm/charts --set image.repository=homelab-test --set image.tag=latest --set image.pullPolicy=Never --namespace homelab --create-namespace'
                }
            }
        }
    }
